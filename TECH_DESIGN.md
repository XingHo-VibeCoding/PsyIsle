...
> Day 5 任务产出 ｜ **v2.0（云端全栈版）** ｜ 2026-09-23
> 上游文档：`PRD.md`（v4.0.1，Day 4）、`research.md`（Day 3）、`云端方案对比与影响评估.md`（决策记录）
> **版本史**：v1.0（2026-09-22）为纯静态路线，已随 commit `f0f4033` 推送；**2026-09-23 老板拍板改走云端全栈**，本文档整体重写为 v2.0。v1.0 全文可从 git 历史追溯（`git show f0f4033:TECH_DESIGN.md`）。
...
**读多写少的两个特例**：呼吸练习（F7）与知识库（F10）**没有 API**——前者是即时体验无需持久化，后者是打包进前端的静态内容。**不是所有数据都值得上云，这是克制。**
...
## 12. 待定问题与临时假设（题面要求：信息不足先列问题）

| # | 问题 | 临时假设（按此设计，可推翻） | 影响面 |
|---|---|---|---|
| 1 | **登录策略** | ✅ **已拍板（2026-09-23）**：**默认匿名登录**（打开页面静默发 uid，无任何注册界面）+ 关于页提供**可选绑定**（绑定后跨设备）。⚠️ 匿名 uid 在清除浏览器数据后不可恢复，此限制要写进关于页 | 首页文案、关于页、auth.js |
| 2 | 云账号归属 | 老板个人腾讯云账号 + 实名，免费体验环境起步 | 开通流程 |
| 3 | 域名 | 先用 CloudBase 默认域名，不买域名 | 上线链接形态 |
| 4 | GitHub 仓库角色 | 仓库继续托管**源代码**（部署产物走 CloudBase，不走 GitHub Pages）；Git 与提交节奏不变 | 仅部署环节 |
| 5 | 旧静态版交付物 | `git` 历史里的 v1.0 原样保留（`f0f4033`），不删不改 | 无 |
...
| **信息不足先列问题** | ✅ | 第 12 节：5 个待定问题 + 临时假设（登录策略为首） |
